...
import extRoleCache, { KEY_EXT_ROLES_CACHE } from '../../RolesTable/utils/ext-role.cache'
...
const RolesModal = ({ visible, onClose, idRole = 0 }: RoleModalProps) => {
  ...
  >(
    ...
    },
    {
      ...
      onMutate: () => {
        return onMutateCache(customer.id)
      },
      onSettled: () => {
        onSettledCache(customer.id)
      },
      onError: (error, _, context: any) => {
        onErrorCache(context, customer.id)
        notification({
          type: 'error',
  ...
  const { refetch: refetchRoles } = useQuery(
    `${KEY_EXT_ROLES_CACHE}_GET_ROLE_BY_ID`,
    async () => {
