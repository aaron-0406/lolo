...
const FileCaseQrModal = ({ isVisible, onClose }: Props) => {
  ...
  const { mutate: CreateQrCode } = useMutation<
    AxiosResponse<QrResponse>,
    AxiosError<CustomErrorResponse>
  >(
    async () => {
      const numberCaseFile = watch('numberCaseFile')
      return await createQrCode(numberCaseFile ?? 0)
    },
    {
      onSuccess: (data) => {
  ...
  return (
    <Modal id="file-case-qr-modal" title="Código QR" onClose={onClose} visible={isVisible} size="small"
      footer={
