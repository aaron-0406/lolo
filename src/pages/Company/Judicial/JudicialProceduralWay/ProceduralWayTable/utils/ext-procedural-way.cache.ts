...
export const KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE = 'key-ext-judicial-procedural-way-cache'
...
const extProceduralWayCache = (queryClient: QueryClient) => {
  const createProceduralWayCache = (data: JudicialProceduralWayType) => {
    queryClient.setQueryData<QueryDataType>([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, data.customerHasBankId], (old) => {
      if (old) {
  ...
  const editProceduralWayCache = (data: JudicialProceduralWayType) => {
    queryClient.setQueryData<QueryDataType>([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, data.customerHasBankId], (old) => {
      if (old) {
        const dataUpdated = old.data.map((court: JudicialProceduralWayType) => {
          if (court.id === data.id) {
            return data
          }
  ...
  const deleteProceduralWayCache = (idUser: string, chb: number) => {
    queryClient.setQueryData<QueryDataType>([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, chb], (old) => {
      if (old) {
  ...
  const onRefetchQueryCache = async (chb: number) => {
    await queryClient.refetchQueries([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, chb])
  }

  const onMutateCache = async (chb: number) => {
    const old = queryClient.getQueryData([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, chb])
    if (!old) {
      await queryClient.prefetchQuery([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, chb])
    }
  ...
  const onSettledCache = (chb: number) => {
    queryClient.cancelQueries([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, chb])
  }

  const onErrorCache = (context: { old: QueryDataType }, chb: number) => {
    queryClient.setQueryData([KEY_EXT_JUDICIAL_PROCEDURAL_WAYS_CACHE, chb], context.old)
  }
