...
import chargesEncumbrancesCache, {
  KEY_JUDICIAL_CHARGES_ENCUMBRANCES_CACHE,
} from '../../JudicialCollateralChargesEncumbrancesTable/utils/judicial-charges-encumbrances.cache'
...
const ChargesEncumbrancesModal = ({ isOpen, onClose, id }: Props) => {
  ...
  const {
    actions: { editChargesEncumbrancesCache, createChargesEncumbrancesCache },
    onMutateCache,
    onSettledCache,
    onErrorCache,
  } = chargesEncumbrancesCache(queryClient)

  const { refetch: refetchJudicialChargesEncumbrances } = useQuery<
  ...
  const { isLoading: isLoadingJudicialChargesEncumbrances, mutate: onCreateJudcialChargesEncumbrancesMutate } =
    useMutation<AxiosResponse<JudicialCollateralChargesEncumbrancesType>, AxiosError<CustomErrorResponse>>(
      async () => {
  ...
  const { isLoading: isLoadingEditJudicialChargesEncumbrances, mutate: onEditJudicialChargesEncumbrancesMutate } =
    useMutation<AxiosResponse<JudicialCollateralChargesEncumbrancesType>, AxiosError<CustomErrorResponse>>(
      async () => {
