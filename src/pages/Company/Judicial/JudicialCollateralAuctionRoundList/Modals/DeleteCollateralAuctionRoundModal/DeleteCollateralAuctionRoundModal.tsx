...
import judicialCollateralAuctionRoundListCache from '../../JudicialCollateralAuctionRoundListTable/utils/judicial-collateral-auction-round-list.cache'
  ...
  const { isLoading: loadingDeleteCollateralAuctionRound, mutate: deleteCollateralAuctionRoundMutate } = useMutation<
    ...
    >(
      ...
      },
      {
        ...
        onMutate: () => {
          return onMutateCache(Number(chb))
        },
        onSettled: () => {
          onSettledCache(Number(chb))
        },
        onError: (error, _, context: any) => {
          onErrorCache(context, Number(chb))
          notification({
            type: 'error',
