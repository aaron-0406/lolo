...
yup.setLocale({
  ...
  number: {
    ...
    moreThan: ({ more }: { more: number }) => `el valor debe ser mayor que ${more}`,
  ...
  string: {
    ...
    min: ({ min }: { min: number }) => `el valor mínimo es de ${min} caracteres`,
    max: ({ max }: { max: number }) => `el valor máximo es de ${max} caracteres`,
  },
  array: {
